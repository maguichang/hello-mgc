# -*- coding: utf-8 -*-
"""
Created on Wed Aug 23 10:27:01 2017
    ...
    for k2 in f_id_new:
        ...
        for k3 in t_id_new:
            ...
            for k4 in tk_new:
                ...
                cur.execute(sql6)
                '''
                sql7="create table tp select * from (select faultDays,windfarm,sendtype,rules_id,windfarm_id,turbine,turbine_id,left(trigkeys,11) new_trigkeys,level1low,level1high,level2low,level2high,\
...
level1low,level1high,level2low,level2high,level3low,num1_trigkey,create_time_ma,sendtype,faultDays)\
SELECT rules_id,windfarm_id,windfarm,turbine,turbine_id,new_trigkeys,level1low,level1high,level2low,\
level2high,level3low,numtrigkey,create_time_ma,sendtype,faultDays from tp"
                cur.execute(sql9)
    ...
    if st==0 or st==2:
        ...
        for k11 in id_turbine:
            ...
            if f_new[0]==1 and t_end_new > sd_new:
                ...
                #sql22="call in_result("+str(k11[0])+")"
                
                
                
                sql22="INSERT INTO eg_bus_alert_result(alert_name,alert_level,alert_desc,alert_location,alert_type,\
windfarm,alert_date,create_time,turbine,fault_code,rules_id) SELECT alert_name_ma,alert_level_ma1,\
alert_desc_ma,alert_location_ma,alert_type_ma,windfarm_id,alert_date_ma,create_time_ma,turbine,\
fault_code_ma,rules_id from ma WHERE alert_level_ma1 is not null and id= "+str(k11[0])+" and turbine_id= "+str(k11[1])
                ...
                if c_new>0:
                    ...
                    sql18="INSERT INTO eg_bus_alert_result(alert_name,alert_level,alert_desc,alert_location,alert_type,\
windfarm,alert_date,create_time,turbine,fault_code,rules_id) SELECT alert_name_ma,alert_level_ma1,\
alert_desc_ma,alert_location_ma,alert_type_ma,windfarm_id,alert_date_ma,create_time_ma,turbine,\
fault_code_ma,rules_id from ma WHERE alert_level_ma1 is not null and id= "+str(k11[0])+" and turbine_id= "+str(k11[1])+" and sendtype!=1"
    ...
    #sd_new=[f[i][0] for i in xrange(len(sd))]#now()-faultdays的值，查询起始时间值
                ...
                if f_new2[0]==1 and t_end_new2 > sd_new2:
                    sql22_2="INSERT INTO eg_bus_alert_result(alert_name,alert_level,alert_desc,alert_location,alert_type,\
windfarm,alert_date,create_time,turbine,fault_code,rules_id) SELECT alert_name_ma,alert_level_ma1,\
alert_desc_ma,alert_location_ma,alert_type_ma,windfarm_id,alert_date_ma,create_time_ma,turbine,\
fault_code_ma,rules_id from ma WHERE alert_level_ma1 is not null and windfarm_id= "+str(wf)
                    cur.execute(sql22_2)
                    conn.autocommit(True)
                else:
                     ...
                     if c_new2>0:
                         #sql18="call insert_result2()"
                         sql22_3="INSERT INTO eg_bus_alert_result(alert_name,alert_level,alert_desc,alert_location,alert_type,\
windfarm,alert_date,create_time,turbine,fault_code,rules_id) SELECT alert_name_ma,alert_level_ma1,\
alert_desc_ma,alert_location_ma,alert_type_ma,windfarm_id,alert_date_ma,create_time_ma,turbine,\
fault_code_ma,rules_id from ma WHERE alert_level_ma1 is not null and windfarm_id= "+str(wf)
                         cur.execute(sql22_3)
